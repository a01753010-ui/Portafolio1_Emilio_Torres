...
#Agregamos attendance_percent como nueva variable predictora ya que está en el 4to lugar con mayor correlación y se encuentra en ambos datasets
X = df[['previous_grade', 'study_time_hours', 'attendance_percent']]
...
y_nuevo_bin = (y >= 80).astype(int)
pred = (X >= 80).astype(int)
...
df_test['Attendance (%)']   = df_test['Attendance (%)'].fillna(X_train['attendance_percent'].mean())

# Seleccionar variables y renombrarlas para que coincidan con los nombres que el scaler y el modelo aprendieron
X_nuevo = df_test[['PreviousGrade', 'Study Hours', 'Attendance (%)']].rename(columns={
